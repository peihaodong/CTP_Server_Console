...
ThreadOneMin::ThreadOneMin()
	:m_pTimerOneMin(nullptr)
{
	//连接redis数据库
	bool bRt = false;
#ifdef _DEBUG
...
#endif 
	if (bRt)	
		std::cout << "1分钟线程redis连接成功" << std::endl;
	else
		std::cout << "1分钟线程redis连接失败" << std::endl;
}
...
ThreadOneMin::~ThreadOneMin()
{
	if (m_pTimerOneMin)
	{
		delete m_pTimerOneMin;
		m_pTimerOneMin = nullptr;
	}
}

void ThreadOneMin::CloseThread()
{
	//关闭定时器
	if (m_pTimerOneMin->isActive())//判断定时器是否运行
		m_pTimerOneMin->stop();	//停止定时器
}

void ThreadOneMin::run()
{
	std::cout << "1分钟线程定时器开启" << std::endl;

	//初始化定时器
	m_pTimerOneMin = new QTimer;
	//设置定时器是否为单次触发。默认为 false 多次触发
	m_pTimerOneMin->setSingleShot(false);
	//定时器触发信号槽
	connect(m_pTimerOneMin, SIGNAL(timeout()), this, SLOT(slotTimerOneMin()), Qt::DirectConnection);
	//开启定时器
	m_pTimerOneMin->start(60 * 1000);//启动或重启定时器, 并设置定时器时间：毫秒
	//m_pTimerOneMin->start(1000);

	//开启事件循环
	exec();	
}

void ThreadOneMin::slotTimerOneMin()
{
	QDateTime curTime = QDateTime::currentDateTime();
	QString strCurDateTime = curTime.toString("yyyy.MM.dd hh:mm:ss");	//当前日期字符串
	std::cout << strCurDateTime.toStdString() << "  1分钟线程定时器触发，开始加工csv文件数据.............................." << std::endl;
	...
	for (int i = 0; i < fileInfoList.size(); i++)
	{
		...
		std::cout << strCurDateTime.toStdString() << "  1分钟线程：开始加工 " << strFileName.toStdString() << " csv数据" << std::endl;
		QTime time;
		time.start();

		//读取redis数据库中该合约代码最后一条数据的时间
		QString strRedisLastDateTime = GetRedisLastDateTimeOneMin(strFileName);
		//加工csv文件中一分钟的数据
		std::vector<QString> vecProcessData;
		ProcessCsvDataOneMin(strFilePath, strRedisLastDateTime, vecProcessData);

		std::cout << "加工时长：" << time.elapsed() / 1000.0 << "s" << std::endl;
		if (vecProcessData.size() == 0)
		{
			std::cout << strCurDateTime.toStdString() << "  1分钟线程：" << strFileName.toStdString() << " 没有加工出数据，请查看csv文件中是否有新数据" << std::endl;
			continue;
		}
		std::cout << strCurDateTime.toStdString() << "  1分钟线程：" << strFileName.toStdString() << " 加工出数据" << vecProcessData.size() << "条" << std::endl;
		
		//一分钟数据添加到redis数据库
		bool bRt = AppendToRedisOneMin(strFileName,vecProcessData);
		if (bRt)
			std::cout << strCurDateTime.toStdString() << "  1分钟线程：" << strFileName.toStdString() << " 数据全部添加到redis中" << std::endl;
		else
			std::cout << strCurDateTime.toStdString() << "  1分钟线程：" << strFileName.toStdString() << " 部分或全部数据没有添加到redis中" << std::endl;
	}
}

void ThreadOneMin::ProcessCsvDataOneMin(const QString& strFilePath, const QString& strRedisLastDateTime,
	std::vector<QString>& vecProcessData) const
{
	...
	QString strCsvLastMinTime;	//csv文件最后一分钟时间
	...
	for (int i = nCountRow-2; i >= 0; i--)
	{
		...
		if (i == nCountRow-2)
		{
			strCsvLastMinTime = strCsvTime;
			strCsvLastMinTime = strCsvLastMinTime.left(strCsvLastMinTime.size() - 2);
			continue;
		}
		else
		{
			QString strTemp = strCsvTime;
			strTemp = strTemp.left(strTemp.size() - 2);
			if (strCsvLastMinTime == strTemp)
				continue;
		}
	...
	for (int i = 0; i < deqCsvData.size(); i++)
	{
		...
		if (!IsItInTime(strCsvDateTime, strDateTimeStart, strDateTimeEnd))
		{
			...
			strDateTimeEnd = datetimeTemp.addSecs(60).toString("yyyy.MM.dd hh:mm:ss");
		}
		//添加数据
...
bool ThreadOneMin::AppendToRedisOneMin(const QString& strHydm, const std::vector<QString>& vecProcessData)
{
	//切换到db0数据库
	if (!m_redis.SwitchDb(0))
		return false;
...
QString ThreadOneMin::GetRedisLastDateTimeOneMin(const QString& strHydm)
{
	...
	{//redis中有数据
		...
		QString strEndTime = listCellData[3];
...
int ThreadOneMin::CompDateTime(const QString& strDateTime1, const QString& strDateTime2) const
{
...
bool ThreadOneMin::IsItInTime(const QString& strDateTime, const QString& strDateTimeStart,
	const QString& strDateTimeEnd) const
{
