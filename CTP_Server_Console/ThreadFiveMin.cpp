...
ThreadFiveMin::ThreadFiveMin()
	: m_pTimerFiveMin(nullptr)
{
	//连接redis数据库
	bool bRt = false;
#ifdef _DEBUG
...
#endif 
	if (bRt)
		std::cout << "5分钟线程redis连接成功" << std::endl;
	else
		std::cout << "5分钟线程redis连接失败" << std::endl;
}
...
ThreadFiveMin::~ThreadFiveMin()
{
	if (m_pTimerFiveMin)
	{
		delete m_pTimerFiveMin;
		m_pTimerFiveMin = nullptr;
	}
}

void ThreadFiveMin::CloseThread()
{
	//关闭定时器
	if (m_pTimerFiveMin->isActive())//判断定时器是否运行
		m_pTimerFiveMin->stop();	//停止定时器
}

void ThreadFiveMin::run()
{
	std::cout << "5分钟线程定时器开启" << std::endl;

	//初始化定时器
	m_pTimerFiveMin = new QTimer;
	//设置定时器是否为单次触发。默认为 false 多次触发
	m_pTimerFiveMin->setSingleShot(false);
	//定时器触发信号槽
	connect(m_pTimerFiveMin, SIGNAL(timeout()), this, SLOT(slotTimerFiveMin()), Qt::DirectConnection);
	//开启定时器
	m_pTimerFiveMin->start(60 * 1000 * 5);//启动或重启定时器, 并设置定时器时间：毫秒
	//m_pTimerFiveMin->start(1000 * 5);

	//开启事件循环
	exec();
}

void ThreadFiveMin::slotTimerFiveMin()
{
	QDateTime curTime = QDateTime::currentDateTime();
	QString strCurDateTime = curTime.toString("yyyy.MM.dd hh:mm:ss");	//当前日期字符串
	std::cout << strCurDateTime.toStdString() << "  5分钟线程定时器触发，开始加工csv文件数据.............................." << std::endl;
	...
	for (int i = 0; i < fileInfoList.size(); i++)
	{
		...
		std::cout << strCurDateTime.toStdString() << "  5分钟线程：开始加工 " << strFileName.toStdString() << " csv数据" << std::endl;
		QTime time;
		time.start();

		//读取redis数据库中该合约代码最后一条数据的时间
		QString strRedisLastDateTime = GetRedisLastDateTimeFiveMin(strFileName);
		//加工csv文件中五分钟的数据
		std::vector<QString> vecProcessData;
		ProcessCsvDataFiveMin(strFilePath, strRedisLastDateTime, vecProcessData);

		std::cout << "加工时长：" << time.elapsed() / 1000.0 << "s" << std::endl;
		if (vecProcessData.size() == 0)
		{
			std::cout << strCurDateTime.toStdString() << "  5分钟线程：" << strFileName.toStdString() << " 没有加工出数据，请查看csv文件中是否有新数据" << std::endl;
			continue;
		}
		std::cout << strCurDateTime.toStdString() << "  5分钟线程：" << strFileName.toStdString() << " 加工出数据" << vecProcessData.size() << "条" << std::endl;

		//五分钟数据添加到redis数据库
		bool bRt = AppendToRedisFiveMin(strFileName, vecProcessData);
		if (bRt)
			std::cout << strCurDateTime.toStdString() << "  5分钟线程：" << strFileName.toStdString() << " 数据全部添加到redis中" << std::endl;
		else
			std::cout << strCurDateTime.toStdString() << "  5分钟线程：" << strFileName.toStdString() << " 部分或全部数据没有添加到redis中" << std::endl;
	}
}

void ThreadFiveMin::ProcessCsvDataFiveMin(const QString& strFilePath, const QString& strRedisLastDateTime,
	...
	for (int i = 0; i < deqCsvData.size(); i++)
	{
		...
		{//重新赋值时间字符串
			if (strDateTimeEnd == "")
			{
				...
				strDateTimeEnd = datetimeTemp.addSecs(60 * 5).toString("yyyy.MM.dd hh:mm:ss");
			}
			else
			{
...
bool ThreadFiveMin::AppendToRedisFiveMin(const QString& strHydm, const std::vector<QString>& vecProcessData)
{
	//切换到db1数据库
	if (!m_redis.SwitchDb(1))
		return false;
...
QString ThreadFiveMin::GetRedisLastDateTimeFiveMin(const QString& strHydm)
{
...
int ThreadFiveMin::CompDateTime(const QString& strDateTime1, const QString& strDateTime2) const
{
...
bool ThreadFiveMin::IsItInTime(const QString& strDateTime, const QString& strDateTimeStart, const QString& strDateTimeEnd) const
{
