...
ThreadFourHour::ThreadFourHour()
	:m_pTimerFourHour(nullptr)
{
	//连接redis数据库
	bool bRt = false;
#ifdef _DEBUG
...
#endif 
	if (bRt)
		std::cout << "4小时线程redis连接成功" << std::endl;
	else
		std::cout << "4小时线程redis连接失败" << std::endl;
}
...
ThreadFourHour::~ThreadFourHour()
{
	if (m_pTimerFourHour)
	{
		delete m_pTimerFourHour;
		m_pTimerFourHour = nullptr;
	}
}

void ThreadFourHour::CloseThread()
{
	//关闭定时器
	if (m_pTimerFourHour->isActive())//判断定时器是否运行
		m_pTimerFourHour->stop();	//停止定时器
}

void ThreadFourHour::run()
{
	std::cout << "4小时线程定时器开启" << std::endl;

	//初始化定时器
	m_pTimerFourHour = new QTimer;
	//设置定时器是否为单次触发。默认为 false 多次触发
	m_pTimerFourHour->setSingleShot(false);
	//定时器触发信号槽
	connect(m_pTimerFourHour, SIGNAL(timeout()), this, SLOT(slotTimerFourHour()), Qt::DirectConnection);
	//开启定时器
	m_pTimerFourHour->start(60 * 1000 * 60 * 4);//启动或重启定时器, 并设置定时器时间：毫秒
	//m_pTimerFourHour->start(1000 * 60 * 4);

	//开启事件循环
	exec();
}

void ThreadFourHour::slotTimerFourHour()
{
	QDateTime curTime = QDateTime::currentDateTime();
	QString strCurDateTime = curTime.toString("yyyy.MM.dd hh:mm:ss");	//当前日期字符串
	std::cout << strCurDateTime.toStdString() << "  4小时线程定时器触发，开始加工csv文件数据.............................." << std::endl;
	...
	for (int i = 0; i < fileInfoList.size(); i++)
	{
		...
		std::cout << strCurDateTime.toStdString() << "  4小时线程：开始加工 " << strFileName.toStdString() << " csv数据" << std::endl;
		QTime time;
		time.start();

		//读取redis数据库中该合约代码最后一条数据的时间
		QString strRedisLastDateTime = GetRedisLastDateTimeFourHour(strFileName);
		//加工csv文件中4小时的数据
		std::vector<QString> vecProcessData;
		ProcessCsvDataFourHour(strFilePath, strRedisLastDateTime, vecProcessData);

		std::cout << "加工时长：" << time.elapsed() / 1000.0 << "s" << std::endl;
		if (vecProcessData.size() == 0)
		{
			std::cout << strCurDateTime.toStdString() << "  4小时线程：" << strFileName.toStdString() << " 没有加工出数据，请查看csv文件中是否有新数据" << std::endl;
			continue;
		}
		std::cout << strCurDateTime.toStdString() << "  4小时线程：" << strFileName.toStdString() << " 加工出数据" << vecProcessData.size() << "条" << std::endl;

		//4小时数据添加到redis数据库
		bool bRt = AppendToRedisFourHour(strFileName, vecProcessData);
		if (bRt)
			std::cout << strCurDateTime.toStdString() << "  4小时线程：" << strFileName.toStdString() << " 数据全部添加到redis中" << std::endl;
		else
			std::cout << strCurDateTime.toStdString() << "  4小时线程：" << strFileName.toStdString() << " 部分或全部数据没有添加到redis中" << std::endl;
	}
}

void ThreadFourHour::ProcessCsvDataFourHour(const QString& strFilePath, const QString& strRedisLastDateTime,
	...
	for (int i = 0; i < deqCsvData.size(); i++)
	{
		...
		{//重新赋值时间字符串
			if (strDateTimeEnd == "")
			{
				...
				strDateTimeEnd = datetimeTemp.addSecs(60 * 60 * 4).toString("yyyy.MM.dd hh:mm:ss");
			}
			else
			{
...
bool ThreadFourHour::AppendToRedisFourHour(const QString& strHydm, const std::vector<QString>& vecProcessData)
{
	//切换到db5数据库
	if (!m_redis.SwitchDb(5))
		return false;
...
QString ThreadFourHour::GetRedisLastDateTimeFourHour(const QString& strHydm)
{
...
int ThreadFourHour::CompDateTime(const QString& strDateTime1, const QString& strDateTime2) const
{
...
bool ThreadFourHour::IsItInTime(const QString& strDateTime, const QString& strDateTimeStart, const QString& strDateTimeEnd) const
{
