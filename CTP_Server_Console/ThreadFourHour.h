...
*/
class ThreadFourHour : public QThread
{
	Q_OBJECT
...
private slots:
	//	定时器对应槽函数
	void slotTimerFourHour();

private:

	//	加工csv文件中4小时的数据
	void ProcessCsvDataFourHour(const QString& strFilePath, const QString& strRedisLastDateTime,
		std::vector<QString>& vecProcessData) const;

	//	4小时数据添加到redis数据库
	bool AppendToRedisFourHour(const QString& strHydm, const std::vector<QString>& vecProcessData);

private:

	//	读取redis数据库中该合约代码最后一条数据的结束时间
	//（如果redis中没有该合约代码的数据，返回""）
	QString GetRedisLastDateTimeFourHour(const QString& strHydm);
