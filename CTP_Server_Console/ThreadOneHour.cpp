...
#endif 
	if (bRt)
		std::cout << "1小时线程redis连接成功" << std::endl;
	else
		std::cout << "1小时线程redis连接失败" << std::endl;
}
...
{
	//关闭定时器
	if (m_pTimerOneHour->isActive())//判断定时器是否运行
		m_pTimerOneHour->stop();	//停止定时器
}

void ThreadOneHour::run()
{
	std::cout << "1小时线程定时器开启" << std::endl;
	...
	//开启定时器
	m_pTimerOneHour->start(60 * 1000 * 60);//启动或重启定时器, 并设置定时器时间：毫秒
...
void ThreadOneHour::slotTimerOneHour()
{
	...
	std::cout << strCurDateTime.toStdString() << "  1小时线程定时器触发，开始加工csv文件数据.............................." << std::endl;
	...
	for (int i = 0; i < fileInfoList.size(); i++)
	{
		...
		std::cout << strCurDateTime.toStdString() << "  1小时线程：开始加工 " << strFileName.toStdString() << " csv数据" << std::endl;
		QTime time;
		time.start();
		...
		{
			std::cout << strCurDateTime.toStdString() << "  1小时线程：" << strFileName.toStdString() << " 没有加工出数据，请查看csv文件中是否有新数据" << std::endl;
			continue;
		}
		std::cout << strCurDateTime.toStdString() << "  1小时线程：" << strFileName.toStdString() << " 加工出数据" << vecProcessData.size() << "条" << std::endl;

		//1小时数据添加到redis数据库
		bool bRt = AppendToRedisOneHour(strFileName, vecProcessData);
		if (bRt)
			std::cout << strCurDateTime.toStdString() << "  1小时线程：" << strFileName.toStdString() << " 数据全部添加到redis中" << std::endl;
		else
			std::cout << strCurDateTime.toStdString() << "  1小时线程：" << strFileName.toStdString() << " 部分或全部数据没有添加到redis中" << std::endl;
	}
}
